...
export class MoviedetailsComponent implements OnInit, OnDestroy {
  ...
  constructor(private router: Router, private bokingService: BookingserviceService
              , private activatedRoute: ActivatedRoute, private moviefetch: MovieFetchService,
              private theaterFetch: TheaterFetchService, private locationSer: LocationChangeService) {
  ...
  fetchTheaters(loc: string) {
    this.TheaterFetchSer2 = this.theaterFetch.fetchTheaters(loc).subscribe((res: Theater[]) => {
      this.theater$ = res;
    });
